...
import os
import math
import esperti  # Il modulo che gestisce i dati esterni
import uvicorn  # Necessario per avviare il server
...
async def predict_match(request: MatchRequest):
    ...
    # Integrazione Panel Esperti protetta
    panel_esperti = {}
    try:
        panel_esperti = await esperti.get_tutti_esperti(request.match_id)
    except Exception as e:
        print(f"Errore caricamento esperti: {e}")
